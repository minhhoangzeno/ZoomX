...
const mongoose = require('mongoose'),
    HeroZoomX = mongoose.model('heroZoomX'),
...
exports.update_hero = (req, res) => {
    ...
    HeroZoomX.findById(id).exec().then(hero => {
        ImageUtil.updateSingeFile(req.files[0], hero.imageCover, 'Hero/ZoomX').then(() => {
            HeroZoomX.findByIdAndUpdate(id, {
                title: req.body.title,
                label: req.body.label
            }).then(data => {
                res.send(data)
...
exports.delete_hero = (req, res) => {
    let id = req.params.hero_id;
    HeroZoomX.findById(id).exec().then(async hero => {
