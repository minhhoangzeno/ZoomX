...
exports.update_hero = (req, res) => {
    ...
    HeroRecruitment.findById(id).exec().then(hero => {
        ImageUtil.updateSingeFile(req.files[0], hero.imageCover, 'Hero/Recruitment').then(() => {
            HeroRecruitment.findByIdAndUpdate(id, {
                title: req.body.title,
                label: req.body.label
            }).then(data => {
                res.send(data)
...
exports.delete_hero = (req, res) => {
    let id = req.params.hero_id;
    HeroRecruitment.findById(id).exec().then(async hero => {
