...
const mongoose = require('mongoose'),
    HeroNews = mongoose.model('heroNews'),
...
exports.update_hero = (req, res) => {
    ...
    HeroNews.findById(id).exec().then(hero => {
        ImageUtil.updateSingeFile(req.files[0], hero.imageCover, 'Hero/News').then(() => {
            HeroNews.findByIdAndUpdate(id, {
                title: req.body.title,
                label: req.body.label
            }).then(data => {
                res.send(data)
...
exports.delete_hero = (req, res) => {
    let id = req.params.hero_id;
    HeroNews.findById(id).exec().then(async hero => {
