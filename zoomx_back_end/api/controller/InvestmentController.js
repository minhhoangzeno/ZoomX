...
exports.add_investment = (req, res) => {
  ...
  Promise.all([uploadHero, uploadCover])
    .then((result) => {
      Investment.create({
...
exports.update_investment = async (req, res) => {
  ...
  let updateHero = new Promise((resolve, reject) => {
    ImageUtil.updateSingeFile(fileHero, investment.imageHero, "Investment")
      .then((result) => {
        resolve(result);
      })
      .catch((error) => {
  ...
  Promise.all([updateCover, updateHero])
    .then((result) => {
