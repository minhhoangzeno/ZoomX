    ...
    .then((result) => {
      ZoomX.create({
        ...
        imageCover: result.filter((item) => item.imageCover)[0].imageCover
          ...
          : null,
        profile: result.filter((item) => item.profile)[0].profile
          ? result.filter((item) => item.profile)[0].profile
          : null,
      })
        .then((data) => {
          res.send(data);
...
exports.update_zoomx = async (req, res) => {
  ...
  let updateCover = new Promise((resolve, reject) => {
    ImageUtil.updateSingeFile(fileCover, zoomx.imageCover, "ZoomX")
      .then((result) => {
        resolve(result);
      })
      .catch((error) => {
  ...
  let updateFile = new Promise((resolve, reject) => {
    FileUtil.updateSingeFile(filePDF, zoomx.profile, "ZoomX")
      .then((result) => {
        resolve(result);
      })
      .catch((error) => {
...
exports.delete_zoomx = async (req, res) => {
  ...
  let deleteImageCover = new Promise((resolve, reject) => {
    ImageUtil.deleteSingleFile(zoomx.imageCover)
      .then((result) => {
        resolve(result);
      })
      .catch((error) => {
  ...
  let deleteFileBook = new Promise((resolve, reject) => {
    FileUtil.deleteSingleFile(zoomx.profile)
      .then((result) => {
        resolve(result);
      })
      .catch((error) => {
