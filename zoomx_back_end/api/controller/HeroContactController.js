...
const mongoose = require('mongoose'),
    HeroContact = mongoose.model('heroContact'),
...
exports.update_hero = (req, res) => {
    ...
    HeroContact.findById(id).exec().then(hero => {
        ImageUtil.updateSingeFile(req.files[0], hero.imageCover, 'Hero/Contact').then(() => {
            HeroContact.findByIdAndUpdate(id, {
                title: req.body.title,
                label: req.body.label
            }).then(data => {
                res.send(data)
...
exports.delete_hero = (req, res) => {
    let id = req.params.hero_id;
    HeroContact.findById(id).exec().then(async hero => {
