...
exports.get_libraryvideo = async (req, res) => {
    ...
    let totalPage;
    await LibraryVideo.find().then(result => {
        totalPage = result
    }).catch(error => {
        console.log(error)
    })
    LibraryVideo
        ...
        .limit(perPage)
        .exec((err, data) => {
            LibraryVideo.countDocuments((err, count) => { // đếm để tính có bao nhiêu trang
...
exports.delete_library_video = (req, res) => {
   LibraryVideo.findByIdAndDelete(req.params.library_video_id).exec()
   .then(data => {
       res.send(data)
   }).catch(err => {
       res.send(err)
