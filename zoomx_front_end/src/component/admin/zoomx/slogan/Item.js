...
export default function Item({ dataSlogan ,indexNum, getSlogan, handleLoading }) {
    const [modalShow, setModalShow] = React.useState(false);
    const deleteSlogan = async (Slogan_id) => {
        handleLoading(true)
        const path = `/slogan/${Slogan_id}`;
        try {
        ...
        <>
            <tr >
                ...
                <td style={{ display: 'flex', justifyContent: 'space-around' }}>
                   ...
                   />
                    <button onClick={() => deleteSlogan(dataSlogan._id)}>
                        <svg style={{ width: 24, height: 24 }} viewBox="0 0 24 24">
