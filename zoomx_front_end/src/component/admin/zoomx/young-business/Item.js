...
export default function Item({ dataYoungBusiness ,indexNum, getYoungBusiness, handleLoading }) {
    const [modalShow, setModalShow] = React.useState(false);
    const deleteYoungBusiness = async (youngbusiness_id) => {
        handleLoading(true)
        const path = `/youngbusiness/${youngbusiness_id}`;
        try {
        ...
        <>
            <tr >
                ...
                <td className="text-center" style={{verticalAlign:'middle'}}>
                    <img alt="" src={dataYoungBusiness?.imageYoung?.url} style={{width:100,height:'auto'}} />
                </td>
                <td style={{ display: 'flex', justifyContent: 'space-around' }}>
                   ...
                   dataYoungBusiness={dataYoungBusiness}
                   getYoungBusiness={getYoungBusiness}
                   handleLoading={handleLoading}
                   />
                    <button onClick={() => deleteYoungBusiness(dataYoungBusiness._id)}>
                        <svg style={{ width: 24, height: 24 }} viewBox="0 0 24 24">
