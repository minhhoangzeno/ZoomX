...
export default function Item({
  ...
  const deleteReasonSelect = async (reason_select_id) => {
    handleLoading(true);
    const path = `/reason-select/${reason_select_id}`;
    try {
  ...
  return (
    <>
      <tr>
        ...
        <td className="text-center" style={{ verticalAlign: "middle" }}>
          {dataReasonSelect?.title}{" "}
        </td>

        <td
          className="text-center"
          style={{ verticalAlign: "middle" }}
          dangerouslySetInnerHTML={{ __html: dataReasonSelect?.content }}
        ></td>

        <td className="text-center" style={{ verticalAlign: "middle" }}>
          <img
            alt=""
            src={dataReasonSelect?.imageCover?.url}
        ...
        <td style={{ verticalAlign: "middle" }} className="btn__setting">
          ...
          />
          <button onClick={() => deleteReasonSelect(dataReasonSelect._id)}>
            <svg
              style={{ width: 24, height: 24 }}
              viewBox="0 0 24 24"
              color="#65676b"
