...
import { deletePartner } from '../../../../lib/api/PartnerAPI';
import ModalPartner from './ModalPartner';

export default function PartnerItem({ dataPartner ,indexNum}) {
    const [modalShow, setModalShow] = React.useState(false);
    return (
        <>
            <tr >
                <td className="text-center" style={{verticalAlign:'middle'}}>{indexNum}</td>
                <td className="text-center" style={{verticalAlign:'middle'}}>{dataPartner?.name}</td>
                <td className="text-center" style={{verticalAlign:'middle'}}>
                    <img alt="" src={dataPartner?.logo?.url} style={{width:50,height:'auto'}} />
                </td>
                <td className="text-center">{dataPartner?.isDeleted ? "Không hoạt động" : "Hoạt động"}</td> 
                <td style={{ display: 'flex', justifyContent: 'space-around' }}>
                    ...
                    />
                    <button onClick={() => deletePartner(dataPartner._id)}>
                        <svg style={{ width: 24, height: 24 }} viewBox="0 0 24 24">
