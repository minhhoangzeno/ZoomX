...
export default function NewsPage() {
  ...
  return (
    <>
      ...
      <HeroPage
        title={hero?.[0]?.title}
        title_sub={hero?.[0]?.label}
        imageBackground={hero?.[0]?.imageCover?.url}
      />
        ...
        <div className="container-fluid main__home">
          {isSearch && (
            <>
              ...
              >
                {data?.totalPage} Kết quả hiện thị cho tìm kiếm &nbsp;{" "}
          ...
          <div className="row item__home">
            <div
              className="col-xl-8 col-6 col-12 main__news__page"
              style={{ paddingLeft: 0 }}
            >
                ...
                <div className="row list__inner">
                  {!loading ? (
                    data?.data?.map((item, index) => {
                      return <ItemNews data={item} key={index} />;
                    })
                  ) : (
                    <Loading />
                  )}
